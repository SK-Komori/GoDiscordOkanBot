...
func main() {
	...
	commandHandlers := map[string]func(s *discordgo.Session, i *discordgo.InteractionCreate){
		"bot-test": func(s *discordgo.Session, i *discordgo.InteractionCreate) {
			...
			// if err != nil {
			// 	log.Panicf("failed to first response: %+v", err)
			// }

			// time.Sleep(time.Second * 3)

			fmt.Println("awawawawaw")
			if option, ok := optionMap["mojiretsu"]; ok {
				respStr = "あなたが入力したのは「" + option.StringValue() + "」ですね！"
			} else {
				respStr = "あなたはなにも入力してないですね；；"
			}
			...
			if err != nil {
				log.Panicf("failed to second response: %+v", err)
			}
			fmt.Println("awawawawaw")
		},
		"bubuduke": func(s *discordgo.Session, i *discordgo.InteractionCreate) {
			...
			if option, ok := optionMap["channel"]; ok {
				...
				if vc.Type == discordgo.ChannelTypeGuildVoice {
					respStr = fmt.Sprintf("%sに参加している人を追い出します！", vc.Name)
	...
	for i, v := range commands {
		cmd, err := ses.ApplicationCommandCreate(ses.State.User.ID, "", v)
		if err != nil {
	...
	if isRemoveCommands {
		...
		for _, v := range registeredCommands {
			err := ses.ApplicationCommandDelete(ses.State.User.ID, "", v.ID)
			fmt.Println(v)
			if err != nil {
